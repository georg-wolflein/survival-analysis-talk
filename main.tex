...
\begin{frame}{Survival function}
    Let $T$ be a continuous random variable representing time.

    The \textbf{survival function} $S(t)$ is the probability that an individual will survive for a time greater than $t$.
    \pause
...
\section[AFT model]{Accelerated failure time (AFT) model}
